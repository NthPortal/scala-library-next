...
trait SetFromMapOps[
  ...
  @inline protected[this] def adaptedTuple[B](f: A => B): ((A, Unit)) => (B, Unit) = { case (elem, _) => f(elem) -> () }
...
object SetFromMapOps {
  ...
  trait Sorted[
    ...
    override def collect[B](pf: PartialFunction[A, B])(implicit @implicitNotFound(SortedSetOps
      .ordMsg) ev: Ordering[B]): CC[B] =
    ...
    override def flatMap[B](f: A => IterableOnce[B])(implicit @implicitNotFound(SortedSetOps
      .ordMsg) ev: Ordering[B]): CC[B] =
      fromSortedMap {
        underlying flatMap {
          case (elem, _) => f(elem) match {
